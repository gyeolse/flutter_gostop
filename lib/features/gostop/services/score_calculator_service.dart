...
class ScoreCalculatorService {
  static Map<String, int> calculateRoundScores({
    ...
    for (final player in players) {
      if (player.id != scoreInput.winnerId) { // 승자는 광 팔기 불가
    ...
    final actualLosers = players.where((p) => 
        p.id != scoreInput.winnerId && 
        !gwangSellers.contains(p.id)).toList();
    ...
    for (final player in players) {
      ...
      // 연뻑 보상 (1인당) 
      if (scoreInput.specialSituations['${player.id}_consecutiveFail'] == true) {
        specialEarningsPerPlayer += gameRules.ppeoPrice;
      }
      ...
      if (scoreInput.specialSituations['${player.id}_tripleFailure'] == true) {
        specialEarningsPerPlayer += gameRules.ppeoPrice * 3;
      }
      ...
      if (specialEarningsPerPlayer > 0) {
        ...
        for (final otherPlayer in otherPlayers) {
          ...
          scores[otherPlayer.id] = (scores[otherPlayer.id] ?? 0) - specialEarningsPerPlayer;
        }
      }
    }
  ...
  static String generateScoreSummary({
    ...
    final otherPlayers = players.where((p) => p.id != scoreInput.winnerId).toList();
    for (final player in otherPlayers) {
      ...
      summary += '$icon ${player.name}: $sign${_formatCurrency(playerAmount)}\n';
      ...
      final gwangCount = gwangSellingCount[player.id] ?? 0;
      if (gwangCount > 0) {
        details.add('광팔기 $gwangCount장 (+${_formatCurrency(gwangCount * gameRules.gwangSellPrice)})');
      }
      ...
      if (scoreInput.specialSituations['${player.id}_tripleFailure'] == true) {
        final totalEarning = gameRules.ppeoPrice * 3 * otherPlayersCount;
      ...
      for (final otherPlayer in players.where((p) => p.id != player.id)) {
        if (scoreInput.specialSituations['${otherPlayer.id}_firstFail'] == true) {
          specialPayments.add('${otherPlayer.name} 첫뻑 (-${_formatCurrency(gameRules.ppeoPrice)})');
        }
        if (scoreInput.specialSituations['${otherPlayer.id}_consecutiveFail'] == true) {
          specialPayments.add('${otherPlayer.name} 연뻑 (-${_formatCurrency(gameRules.ppeoPrice)})');
        }
        if (scoreInput.specialSituations['${otherPlayer.id}_ttaDak'] == true) {
          specialPayments.add('${otherPlayer.name} 따닥 (-${_formatCurrency(gameRules.ddadakPrice)})');
        }
        if (scoreInput.specialSituations['${otherPlayer.id}_tripleFailure'] == true) {
          specialPayments.add('${otherPlayer.name} 삼연뻑 (-${_formatCurrency(gameRules.ppeoPrice * 3)})');
        }
        if (scoreInput.specialSituations['${otherPlayer.id}_president'] == true) {
          specialPayments.add('${otherPlayer.name} 대통령 (-${_formatCurrency(gameRules.presidentPrice)})');
        }
      }
      ...
      if (playerAmount < 0) {
        ...
        if (penalties.isNotEmpty) {
          ...
          final multiplierText = penaltyMultiplier > 1 ? ' ($penaltyMultiplier배)' : '';
          details.add('${penalties.join('+')} (-${_formatCurrency(penaltyAmount)})$multiplierText');
        }
      }
